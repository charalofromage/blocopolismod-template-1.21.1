...
public class LeadChestScreenHandler extends ScreenHandler {
    private final Inventory inventory;

    public LeadChestScreenHandler(int syncId, PlayerInventory playerInventory, PacketByteBuf buf) {
        this(syncId, playerInventory, playerInventory.player.getWorld().getBlockEntity(buf.readBlockPos()));
    }

    public LeadChestScreenHandler(int syncId, PlayerInventory playerInventory, BlockPos pos) {
    ...
    private static final int BG_HEIGHT = 328;  // <-- remplace par la hauteur de ton image
    ...
    public LeadChestScreenHandler(int syncId, PlayerInventory playerInventory, BlockEntity blockEntity) {
        super(ModScreenHandlers.LEAD_CHEST_SCREEN_HANDLER, syncId);
    ...
    private void addPlayerInventory(PlayerInventory playerInventory){
        for (int i = 0; i < 3; ++i){
            for (int l = 0; l < 9; ++l){
                this.addSlot(new Slot(playerInventory, l + i * 9 + 9, 8 + l * 18, 246 + i * 18));
            }
        }
    }
    ...
    private void addPlayerHotbar(PlayerInventory playerInventory){
        for (int i = 0; i < 9; ++i){
            this.addSlot(new Slot(playerInventory, i,  8 + i * 18, 304));
        }
    }
